...
void SingleEqBandProcessor::updateBand()
{
    if (sampleRate > 0)
    {
        ...
        case LowShelf:
            newCoefficients =
                dsp::IIR::Coefficients<float>::makeLowShelf(sampleRate, band.frequency, band.quality, band.gain);
            break;
        case BandPass:
        ...
        case Peak:
            newCoefficients =
                dsp::IIR::Coefficients<float>::makePeakFilter(sampleRate, band.frequency, band.quality, band.gain);
            break;
        case HighShelf:
            newCoefficients =
                dsp::IIR::Coefficients<float>::makeHighShelf(sampleRate, band.frequency, band.quality, band.gain);
            break;
        case HighPass1st:
